#pragma once
#ifndef BOUNDING_BOX3_H_
#define BOUNDING_BOX3_H_
...
bool BoundingBox<T, 3>::intersects(const Ray3<T>& ray) const {
    ...
    const Vector3<T>& rayInvDir = Vector3<T>(1. / ray.direction.x(), 1. / ray.direction.y(), 1. / ray.direction.z()); //ray.direction.rdiv(1);
...
void BoundingBox<T, 3>::expand(T delta) {
    lowerCorner -= Vector3<T>(delta, delta, delta);
    upperCorner += Vector3<T>(delta, delta, delta);
}
...
Vector3<T> BoundingBox<T, 3>::corner(size_t idx) const {
    ...
    return Vector3<T>(width(), height(), depth()).cwiseProduct(offset[idx]) + midPoint();
}
